...
import { fetchWeather, fetchCryptoPrices, getVisitorIpInfo } from '../services/api';
import { Shortcut, WeatherData, CryptoData, VisitorInfo, Todo } from '../types';
...
const Home: React.FC = () => {
  ...
  const [crypto, setCrypto] = useState<{btc: CryptoData, eth: CryptoData} | null>(null);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col justify-between">
               ...
               <div className="space-y-4 my-4">
                  <div>
                    <div className="text-xs font-bold text-orange-500">비트코인</div>
                    <div className="text-xl font-black">${crypto?.btc.priceUsd || '---'}</div>
                  </div>
                  <div>
                    <div className="text-xs font-bold text-indigo-500">이더리움</div>
                    <div className="text-xl font-black">${crypto?.eth.priceUsd || '---'}</div>
                  </div>
               </div>
               <div className="text-[10px] text-slate-400 font-medium">1,380 KRW/USD 기준</div>
            </div>
            ...
            <div className="md:row-span-2 bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col">
               ...
               />
               <div className="flex-grow space-y-3 overflow-y-auto pr-2 max-h-[300px]">
                  {todos.map(t => (
                    <div key={t.id} className="flex items-center gap-3 group">
                       <input type="checkbox" checked={t.completed} onChange={() => setTodos(todos.map(td => td.id === t.id ? {...td, completed: !td.completed} : td))} className="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                       <span className={`text-sm ${t.completed ? 'line-through text-slate-300' : 'text-slate-600'}`}>{t.text}</span>
                    </div>
                  ))}
               </div>
            </div>
